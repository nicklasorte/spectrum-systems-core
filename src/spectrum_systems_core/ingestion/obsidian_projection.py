...
class ObsidianProjection:
    ...
    def write_ai_query_projection(
        ...
        """Phase H: write vault/AI/<query_id>.md. VIEW ONLY. Advisory.

        The advisory warning appears in the first lines of the file
        (FINDING-H-005 / RT3-006). Markdown is view-only and never read
        as input.
        """
        target_dir = Path(vault_root).resolve() / "AI"
        target_dir.mkdir(parents=True, exist_ok=True)
        query_id = ai_output.get("query_id", "")
        target = target_dir / f"{query_id}.md"
        generated_at = _now_iso()
        raw_response = ai_output.get("raw_response", {}) or {}
        citations = ai_output.get("citations", []) or []
        verified = ai_output.get("verified_citations", []) or []
        unverified = ai_output.get("unverified_citations", []) or []
        lines = [
            "---",
            f"output_id: {ai_output.get('output_id', '')}",
            f"query_id: {query_id}",
            ...
            "---",
            "",
            "# ⚠️ AI output is advisory only",
            "",
            "> AI output is advisory only. Review before acting.",
            "> Generated by AIAdapter. VIEW ONLY. Do not edit.",
            "> Regenerated on every query. Never authoritative.",
            "",
            f"**Task**: `{task_type}`  ",
            f"**Question**: {question}  ",
            f"**Confidence**: {ai_output.get('confidence')}  ",
            f"**Grounded**: {bool(ai_output.get('grounded'))}  ",
            f"**Citations verified**: {len(verified)}/{len(citations)}  ",
            "",
            "## Response",
            "",
        ]
        if "answer" in raw_response:
            lines.append(str(raw_response.get("answer", "")))
        else:
            lines.append("```json")
            lines.append(
        ...
        if citations:
            ...
            for cid in citations:
                marker = "✓" if cid in verified else "✗"
                lines.append(f"- {marker} `{cid}`")
            lines.append("")
        if unverified:
    ...
    def write_certification_projection(
        ...
        First line is the VIEW ONLY banner. Renders all 7 check results.
        """
        vault_path = Path(vault_root).resolve()
        out_dir = vault_path / "Certifications"
        out_dir.mkdir(parents=True, exist_ok=True)
        cert_id = record["certification_id"]
        target = out_dir / f"{cert_id}.md"
        target.write_text(
            self._render_certification_projection(record),
            encoding="utf-8",
        )
        return str(target)
    ...
    def _render_certification_projection(self, record: Dict[str, Any]) -> str:
        ...
        lines.append("---")
        lines.append(f"certification_id: {record.get('certification_id', '')}")
        lines.append(f"paper_id: {record.get('paper_id', '')}")
        lines.append(f"certified_at: {record.get('certified_at', '')}")
        lines.append(f"status: {record.get('status', '')}")
        lines.append(f"total_checks: {record.get('total_checks', '')}")
        lines.append(f"passed_checks: {record.get('passed_checks', '')}")
        lines.append(f"failed_checks: {record.get('failed_checks', '')}")
        lines.append(f"skipped_checks: {record.get('skipped_checks', '')}")
        lines.append(
            f"total_pipeline_cost_usd: {record.get('total_pipeline_cost_usd', 0.0)}"
        )
        lines.append(f"certifier_version: {record.get('certifier_version', '')}")
        lines.append("vault_note_status: projection")
        lines.append("---")
        lines.append("")
        lines.append(f"# Certification {record.get('certification_id', '')}")
        lines.append("")
        lines.append(f"**Paper:** `{record.get('paper_id', '')}`")
        lines.append(f"**Status:** **{record.get('status', '')}**")
        lines.append("")
        lines.append("## Check results")
        lines.append("")
        for check in record.get("check_results") or []:
    ...
    def write_formatted_paper_projection(
        ...
        paper_id = formatted_artifact["paper_id"]
        target = papers_dir / f"{paper_id}.md"
        target.write_text(
            self._render_formatted_paper_projection(formatted_artifact),
            encoding="utf-8",
        )
        return str(target)
    ...
    def _render_formatted_paper_projection(
        ...
        lines.append("---")
        lines.append(f"paper_id: {formatted.get('paper_id', '')}")
        lines.append(
            f"source_revised_draft_id: "
            f"{formatted.get('source_revised_draft_id', '')}"
        )
        lines.append(f"formatted_at: {formatted.get('formatted_at', '')}")
        lines.append(f"publication_status: {meta.get('status', '')}")
        lines.append(f"version: {meta.get('version', '')}")
        lines.append(f"content_hash: {formatted.get('content_hash', '')}")
        lines.append(f"formatter_version: {prov.get('formatter_version', '')}")
        lines.append("vault_note_status: projection")
        lines.append("---")
        lines.append("")
        lines.append(f"# {formatted.get('title') or formatted.get('paper_id', '')}")
        lines.append("")
        if authors:
            lines.append("**Authors:** " + ", ".join(str(a) for a in authors))
            lines.append("")
        abstract = formatted.get("abstract") or ""
        if abstract:
        ...
        for section in sorted(sections, key=lambda s: s.get("order", 0)):
            heading = section.get("heading") or section.get("section_id", "")
        ...
        if references:
            ...
            for ref in references:
                ...
                text = ref.get("formatted_text", "")
                lines.append(f"- **{ref_id}** {text}")
            lines.append("")
        lines.append(
            f"_Citations: {len(citations)} · References: {len(references)}_"
        )
        lines.append("")
        return "\n".join(lines)
