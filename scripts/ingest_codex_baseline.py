#!/usr/bin/env python3
"""Ingest a locally-produced Codex meeting_minutes extraction as a
reference baseline alongside the Opus baseline.

The operator runs GPT-5.5 / Codex locally on a Mac against the canonical
extraction prompt (``workflows/prompts/meeting_minutes_llm.md``) and a
transcript already staged in the data-lake. The model produces a
``meeting_minutes``-shaped JSON file. This script is the gate between
that local output and the pipeline: it validates the JSON against the
``meeting_minutes`` schema, looks up the canonical transcript UUID from
``source_record.json``, and explodes the payload into a JSONL file that
shares the on-disk shape of ``opus_reference_minutes.jsonl`` so the
existing comparison engine can read it without changes::

    <data-lake>/store/processed/meetings/<source_id>/
        reference_baselines/
            opus_reference_minutes.jsonl    (existing — untouched)
            codex_reference_minutes.jsonl   (this script writes)

The script is operator-initiated only — there is no CI workflow, no
OpenAI API integration in this repo, and no autonomous re-extraction.

Fail-closed contract:

* ``input_file_not_found`` / ``invalid_input_json`` — the JSON Nick
  pushed is missing or unparseable.
...
* ``already_ingested`` — ``codex_reference_minutes.jsonl`` already
...
``codex-ref-{source_id}-{extraction_type}-{index}`` so re-ingesting the
SAME input JSON over the SAME source assigns identical ids to identical
items. The namespace is frozen — changing it re-keys every future
Codex baseline.
"""
...
# Workflow tag for every JSONL line's ``provenance.produced_by``. The
# comparison engine reads this off the input artifact's provenance, not
# off the baseline row, but stamping it symmetrically with the Opus
# baseline (``opus_reference_baseline_workflow``) keeps the data-lake
# self-documenting.
PRODUCED_BY = "codex_reference_baseline_workflow"

# Identifier the operator stamps into the source meeting_minutes JSON's
# ``provenance.produced_by`` when Codex is the producer. Accepted but
# not required — the schema gates accept any string.
CODEX_LOCAL_PRODUCED_BY = "codex_local"

# Frozen UUID5 namespace. Re-running the ingest over the same input
# JSON must reproduce identical ``pair_id`` values. Distinct from the
# Opus namespace so a Codex row and an Opus row for the same item slot
# get distinct ids; the two are never confused inside the comparison
# engine.
_CODEX_REF_NAMESPACE = uuid.UUID("4d2b8c7e-6f1a-5b3d-8e9f-2a3b4c5d6e7f")

_OUTPUT_SUBDIR = "reference_baselines"
_OUTPUT_FILENAME = "codex_reference_minutes.jsonl"
...
def _resolve_source_artifact_id(
    ...
    Mirrors ``create_opus_reference_baselines._resolve_source_artifact_id``
    (the same fail-closed contract on the same file) but does NOT
    self-heal by re-running Stage-1 ingestion — the Codex ingest only
    receives Nick's JSON, never the raw transcript, so a missing record
    halts with a clear ``run the normal ingestion first`` message.
    """
    sr_path = _meeting_dir(data_lake, source_id) / "source_record.json"
    if not sr_path.is_file():
        raise CodexIngestError(
            "missing_source_record",
            f"no source_record.json at {sr_path}; run the data-lake's "
            f"normal ingestion (or create_opus_reference_baselines.py) "
            f"for {source_id!r} before ingesting a Codex baseline",
        )
    try:
...
def _extract_payload(
    ...
    """Return ``(payload, meeting_date)`` from Nick's input JSON.
    ...
    The flat shape is the natural output ChatGPT produces from a
...
def build_codex_records(
    *,
    ...
    """Explode ``payload`` into JSONL records mirroring the Opus shape.

    Each extraction-type array becomes one row per item. The row carries
    exactly the fields a real Opus baseline row carries plus
    ``provenance.operator`` so a future audit can identify which human
    pushed the artifact. A content key whose value is not a list HALTs
    (``schema_violation`` is raised upstream by the schema validator;
    this loop's check is the second-line defense for inputs that bypass
    the validator via a JSON quirk).
    """
    ...
    for etype in types:
        ...
        for index, item in enumerate(value):
            ...
            pair_id = str(
                uuid.uuid5(
                    _CODEX_REF_NAMESPACE,
                    f"codex-ref-{source_id}-{etype}-{index}",
                )
            )
            records.append(
                {
                    ...
                    # Codex was given the WHOLE transcript in one paste
                    # — no overlap chunking — so the default strategy
                    # value applies. Stamping it explicitly keeps the
                    # comparison engine's strategy-cross-check happy
                    # against a default-off Haiku artifact.
...
def _resolve_model_from_registry() -> str:
    """Return the Codex model string from ``ai/registry/model_registry.json``.
    ...
    the registered string so a re-keying in the registry flows through
    automatically.
    """
    registry_path = (
    ...
    entry = registry.get("codex_reference_baseline")
    if not isinstance(entry, dict):
        raise CodexIngestError(
            "missing_model",
            "ai/registry/model_registry.json has no "
            "'codex_reference_baseline' entry; add it before ingesting",
        )
    ...
    if not isinstance(model_id, str) or not model_id.strip():
        raise CodexIngestError(
            ...
            "codex_reference_baseline.model_id is missing or empty",
        )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--input-file",
        required=True,
        help="Path to the locally-produced Codex meeting_minutes JSON.",
    )
    parser.add_argument(
        "--source-id",
        required=True,
    ...
    parser.add_argument(
        "--operator",
        required=True,
        help="Identifier of the human pushing this artifact (stamped "
        "into every JSONL row's provenance.operator for audit).",
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Override the model string. By default the script reads "
        "ai/registry/model_registry.json::codex_reference_baseline."
        "model_id so a registry re-key flows through automatically.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # create_opus_reference_baselines.main.
    ...
    except CodexIngestError as exc:
        print(
            json.dumps(
                {
                    "status": "failure",
