#!/usr/bin/env python3
"""Seed (or toggle) the Phase W agenda detection feature flag.

Writes ``<DATA_LAKE_PATH>/store/artifacts/config/
phase_w_agenda_detection_enabled.json``. Idempotent.

Usage::

  python scripts/seed_phase_w_flag.py --enable
  python scripts/seed_phase_w_flag.py --disable
  DATA_LAKE_PATH=/custom/path python scripts/seed_phase_w_flag.py --enable

Rollback: re-run with ``--disable`` and commit the resulting JSON to the
data-lake. All Phase W components short-circuit on this single check.
"""
...
def write_flag(data_lake_path: pathlib.Path, enabled: bool) -> pathlib.Path:
    ...
    target = target_dir / f"{PHASE_W_FLAG_NAME}_enabled.json"
    payload = {
        "artifact_type": "feature_flag",
        "schema_version": "1.0.0",
        "created_at": _now_iso(),
        "flag_name": PHASE_W_FLAG_NAME,
        "enabled": bool(enabled),
        "rollback_instructions": (
            "Set enabled:false and commit to data-lake. All Phase W "
            "components read this flag including the classifier prompt "
            "variant."
        ),
        "provenance": {
            "produced_by": "seed_phase_w_flag",
            "phase": "W",
        },
    }
    target.write_text(
...
def _resolve_data_lake_path(cli_path: str | None) -> pathlib.Path:
    ...
    if not raw:
        print(
            "seed_phase_w_flag: DATA_LAKE_PATH not set and --data-lake "
            "not provided",
            file=sys.stderr,
        )
        sys.exit(2)
